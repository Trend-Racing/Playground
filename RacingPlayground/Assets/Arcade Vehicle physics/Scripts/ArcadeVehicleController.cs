...
public class ArcadeVehicleController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (grounded())
        {
            ...
            if (movementMode == MovementMode.AngularVelocity)
            {
                if (Mathf.Abs(verticalInput) > 0.1f && !handBrakeInput)
                {
            ...
            //brake logic
            if (brakingInput < -0.1f)
            {
                float inputDeceleration = deccelration * Mathf.Abs(brakingInput);
                print("Deceleration value: " + brakingInput);
                rb.angularVelocity = Vector3.Lerp(rb.angularVelocity, Vector3.zero, inputDeceleration * Time.deltaTime);
            }
